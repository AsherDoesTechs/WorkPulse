...
const SHIFTS: Shift[] = [
  ...
  },
  {
    ...
    hasViolation: true,
    violation: "Violates 11-hr rest rule — drag to adjust",
  },
  {
    id: "s4",
    agentId: 3,
  ...
  },
  {
    ...
    hasViolation: true,
    violation: "12-hour shift exceeds daily limit",
  },
  {
    id: "s6",
    agentId: 6,
...
export default function TeamDashboard() {
  ...
  const [filter, setFilter] = useState<
    "all" | "balanced" | "approaching" | "critical"
  >("all");
  ...
  const getAuraStyle = (agent: Agent) => {
    if (agent.status === "critical")
      return { boxShadow: "0 0 0 4px #F1948A40, 0 0 0 8px #F1948A20" };
    if (agent.status === "approaching")
      return { boxShadow: "0 0 0 3px #F39C1240" };
        ...
        {[
          ...
          <button
            ...
            className={`p-4 rounded-xl border-2 transition-all text-left ${
              filter === status
                ? "shadow-md"
                : "border-[#E9ECEF] bg-white hover:shadow-sm"
            }`}
            style={
              filter === status
          ...
          >
            ...
            <p
              ...
              style={{
                color: filter === status ? color : "#1A2B3C",
          ...
          {filtered.map((agent) => (
            ...
            >
              ...
              <div className="text-center">
                <p className="text-xs font-medium text-[#1A2B3C] truncate max-w-[60px]">
                  {agent.name.split(" ")[0]}
                </p>
                <p
          ...
          {critical > 0 && (
            ...
            >
              ...
              <p className="text-sm text-[#1A2B3C]">
                <span className="font-semibold text-[#F1948A]">
                  {critical} agents
                </span>{" "}
                are at critical burnout risk. Click their avatar to intervene
                immediately.
              </p>
            </motion.div>
          )}
      ...
      <div className="bg-white rounded-xl border border-[#E9ECEF] shadow-sm p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px]">
            <thead>
              <tr>
                <th className="text-left p-2 text-xs font-medium text-[#1A2B3C]/40 w-32">
                  Agent
                </th>
                {DAYS.map((day) => (
                  <th
                    key={day}
                    className="p-2 text-xs font-medium text-[#1A2B3C]/40 text-center w-24"
                  >
                    {day}
                  </th>
                ))}
                ...
                <tr key={agent.id} className="border-t border-[#F8F9FA]">
                  <td className="p-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-[#1A2B3C] truncate">
                        {agent.name.split(" ")[0]}
                      </span>
                    </div>
                  </td>
        ...
        <div className="mt-4 flex items-center gap-3 p-3 bg-[#F39C12]/5 border border-[#F39C12]/20 rounded-xl">
          ...
          <p className="text-xs text-[#1A2B3C]">
            ...
            detected. Highlighted shifts violate the{" "}
            <span className="font-semibold">11-hour rest rule</span>. Drag to
            adjust.
          </p>
        </div>
      </div>
